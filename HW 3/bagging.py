...
class SimplifiedBaggingRegressor:
            ...
            for bag in range(self.num_bags):
                if i not in self.indices_list[bag]:
                    list_of_predictions_lists[i].append(
                        self.models_list[bag].predict(self.data[i].reshape(1, -1))[0]
                    )
    ...
    def OOB_score(self):
        ...
        oob_predictions_filtered = [
            prediction for prediction in self.oob_predictions if prediction is not None
        ]
        oob_target_filtered = [
            self.target[i]
        ...
        return np.mean(
            (np.array(oob_predictions_filtered) - np.array(oob_target_filtered)) ** 2
        )  # Your Code Here
